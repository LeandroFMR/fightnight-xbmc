...
def info_servidores():
    ...
    if selfAddon.getSetting("fontes-desportogratis") == "true":
        try:
            mensagemprogresso.update(0,'',pleasewait + '(Desporto Grátis)')
    ...
    if selfAddon.getSetting("fontes-livesoccerhd") == "true":
        try:
            mensagemprogresso.update(6,'',pleasewait + '(LivesoccerHD)')
            if mensagemprogresso.iscanceled(): sys.exit(0)
    ...
    if selfAddon.getSetting("fontes-tvzune") == "true":
        try:
            ...
            if mensagemprogresso.iscanceled(): sys.exit(0)
            tvzunelink=comuns.abrir_url_cookie(TVZuneURL,erro=False)
            tvzunefinal='\n'.join(re.compile('location.href=(.+?)</span>').findall(tvzunelink))
    ...
    if selfAddon.getSetting("fontes-tvpthdorg") == "true":
        try:
            ...
            if mensagemprogresso.iscanceled(): sys.exit(0)
            tvpthdorglink=comuns.clean(comuns.abrir_url(TVPTHDZuukURL,erro=False))
    ...
    if nome=='-COLOR white]Zapping-/COLOR]-': zapping(nome)
        ...
        if selfAddon.getSetting("fontes-tvzune") == "true":
            try:
                ...
                tvzune=re.compile("'(.+?)'.+?<span>" + nometvzune ).findall(tvzunelink)
                if tvzune:
                    for resto in tvzune:
...
#                comecarvideo(streamurl,nomecanal,True,zapping)
        ...
        ##livesoccerhd
            ...
            for chname in yukons:
                ...
                #rtmp://198.144.153.144:443/kuyo<playpath>yukonsftw?id=540499a1c4adb86ab839e9938b6b2be7&pid=322e38322e3131342e3336 <swfUrl>http://yukons.net/yplay.swf <pageUrl>http://yukons.net/embed/373937353642364636453733363637343737/5fb4ee60ca32104a0dccd13baad4e922/650/500
                streamurl='rtmp://198.144.153.144:443/kuyo playPath=' + chname + '?id=7510039183c9233a7e330625c21d641a&pid=322e3232322e3131302e313730 swfUrl=http://yukons.net/yplayerv2.swf live=true conn=S:OK timeout=15 swfVfy=true pageUrl=http://yukons.net'
                comecarvideo(streamurl,nomecanal,True,zapping)
